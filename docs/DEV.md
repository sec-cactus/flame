...
4. **verify**：只看 original + verify_points（及 harness 注入的 act 超时说明与本轮 tool 句柄摘要）。通过 → 交付 act 文本。`retry=false` → 交付 diagnosis。无 JSON → 交付 act。Harness 对阶段 JSON 做**格式硬检查**（必要字段与类型；禁止多余键。执行结束后若 `plan.json` 被写入额外字段，恢复白名单并记入 evidence）。Harness 对 `checks` 做**证据审计**（`evidence.py`：每项 check 须含显式 `path:` / `url:` / `` `command` ``；path 做 workspace 归一化；须存在且本轮 tool 触及；url/command 只认 trace，不出站）。另要求本轮 `answer.md`（或 `.flame/answer.md`）的修改时间不早于本轮 `plan.json`（界面只展示该文件）。审计是 verify 的一环：三项都落定后才采纳 `retry`；仅证据腿失败时强制 `retry=true`。`points_met` 且 `checks` 空 → 亦 `evidence_ok=false`。
