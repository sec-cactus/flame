...
class AgentBackend:
    ...
    def run(
        ...
        *,
        ...
        env = os.environ.copy()
        # Headless: never try to open a browser from a child agent.
        env.setdefault("NO_OPEN_BROWSER", "1")

        mode_bit = f" --mode {mode}" if mode else ""
        self.progress.note(f"spawn {phase.value} {self.bin} --model {self.config.model}{mode_bit}")
        ...
        with subprocess.Popen(
            ...
            def _watchdog() -> None:
                nonlocal timed_out
                if not stop_watchdog.wait(self.config.timeout_sec):
                    timed_out = True
                    try:
        ...
        if not saw_result and not result_text:
            result_text = "".join(stderr_chunks).strip() or f"agent exited {code} without a result event"
            is_error = True
    ...
    def run_parallel(
        ...
        """Run independent agent jobs concurrently. Order of results matches jobs."""
        if not jobs:
            return []
        if len(jobs) == 1:
